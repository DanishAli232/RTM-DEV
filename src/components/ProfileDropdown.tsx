...
const ProfileDropdown: React.FCC<{
  userSession: any;
  // signOutRequested: () => any;
  displayName?: boolean;
  className?: string;
}> = ({ userSession, displayName, className }) => {
  const { data: user } = useUser();
  console.log('🚀 ~ file: ProfileDropdown.tsx:50 ~ userSession:', userSession);

  const signedInAsLabel = useMemo(() => {
    // const email = userSession?.auth?.user.email || undefined;
    // const phone = userSession?.auth?.user.phone || undefined;
    const email = userSession?.email || undefined;
    const phone = userSession?.phone || undefined;
  ...
  const isSuperAdmin = useMemo(() => {
    return userSession?.app_metadata.role === GlobalRole.SuperAdmin;
  }, [userSession]);
    ...
    <DropdownMenu>
      ...
      >
        ...
        <DropdownMenuItem
          ...
          onClick={() => {
            localStorage.removeItem('RTM_DEV');
